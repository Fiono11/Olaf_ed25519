...
use crate::{
    scalar_from_canonical_bytes, simplpedpop::SPPOutput, SecretKey, ThresholdPublicKey,
    VerifyingShare, COMPRESSED_EDWARDS_LENGTH, GENERATOR, SCALAR_LENGTH,
};
use alloc::vec::Vec;
    ...
    pub(super) fn verify(
        ...
        (GENERATOR * self.share)
            == (group_commitment_share.0
                + (CompressedEdwardsY::from_slice(&verifying_share.0.to_bytes())
                    .unwrap()
                    .decompress()
                    .unwrap()
                    * challenge
