...
impl SigningKeypair {
    ...
    pub fn sign(
        ...
        let own_verifying_share = VerifyingShare(self.verifying_key);
    ...
    fn compute_signature_share(
        ...
        let z_share: Scalar = signer_nonces.hiding.0
            ...
            + (lambda_i * Scalar::from_canonical_bytes(self.secret_key).unwrap() * challenge);
...
pub fn aggregate(signing_packages: &[SigningPackage]) -> Result<Signature, FROSTError> {
    ...
    let verification_result = threshold_public_key
        ...
        .verify(
            message,
            &ed25519_dalek::Signature::from_bytes(&group_signature.to_bytes()),
        )
